    ...
    def _deal_request(self):
        ...
        try:
            ...
            if ec != error_codes.EC_SUCCESS:
                ...
                return res
            res["code"] = code
            res["next_req_interval"] = SysConfig.current().get(sys_config.SC_VERIFY_CODE_FREQ_SECS)
        except Exception as e:
        ...
        try:
            if code_type == 3:  # 共享宠物
                mob = arg2[:4] + "****" + arg2[-3:]
                msg = {"pet": arg1, "code": code, "mob": mob}
                sms_msg = push_msg.combine_sms_msg[type_defines.MSG_TMPL_SHARE_PET](msg)
                yield msg_rpc.send_sms(sms_msg["sms_template_code"], phone_num, sms_msg["sms_param"])
            else:
